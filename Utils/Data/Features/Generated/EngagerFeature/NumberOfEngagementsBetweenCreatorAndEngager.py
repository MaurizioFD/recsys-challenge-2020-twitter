...
def compute(train, test):
    ...
    result = pd.DataFrame(
        [
            dictionary.get((engager, creator), 0) + dictionary.get((creator, engager), 0)
            for engager, creator
...
class NumberOfEngagementsBetweenCreatorAndEngagerAbstract(GeneratedFeaturePickle, ABC):

    def __init__(self, dataset_id: str):

        super().__init__("number_of_engagements_between_creator_and_engager_" + self._get_suffix(), dataset_id)
        self.pck_path = pl.Path(
            f"{Feature.ROOT_PATH}/{self.dataset_id}/generated/number_of_engagements_between_creator_and_engager/{self.feature_name}.pck.gz")
        self.csv_path = pl.Path(
            f"{Feature.ROOT_PATH}/{self.dataset_id}/generated/number_of_engagements_between_creator_and_engager/{self.feature_name}.csv.gz")
...
class NumberOfEngagementsBetweenCreatorAndEngagerPositive(NumberOfEngagementsBetweenCreatorAndEngagerAbstract):
...
class NumberOfEngagementsBetweenCreatorAndEngagerNegative(NumberOfEngagementsBetweenCreatorAndEngagerAbstract):
...
class NumberOfEngagementsBetweenCreatorAndEngagerLike(NumberOfEngagementsBetweenCreatorAndEngagerAbstract):
...
class NumberOfEngagementsBetweenCreatorAndEngagerRetweet(NumberOfEngagementsBetweenCreatorAndEngagerAbstract):
...
class NumberOfEngagementsBetweenCreatorAndEngagerReply(NumberOfEngagementsBetweenCreatorAndEngagerAbstract):
...
class NumberOfEngagementsBetweenCreatorAndEngagerComment(NumberOfEngagementsBetweenCreatorAndEngagerAbstract):
