...
def compute(train, test):
    dictionary = pd.DataFrame({"count": train.groupby("mapped_feature_tweet_id").size()}).to_dict()["count"]
    result = pd.DataFrame(test["mapped_feature_tweet_id"].map(lambda x: dictionary.get(x, 0)), index=test.index)
    return result


class TweetNumberOfEngagementsAbstract(GeneratedFeaturePickle, ABC):

    def __init__(self, dataset_id: str):

        super().__init__("tweet_number_of_" + self._get_suffix() + "_engagements", dataset_id)
        self.pck_path = pl.Path(
            f"{Feature.ROOT_PATH}/{self.dataset_id}/generated/tweet_features/tweet_number_of_engagements/{self.feature_name}.pck.gz")
        self.csv_path = pl.Path(
            f"{Feature.ROOT_PATH}/{self.dataset_id}/generated/tweet_features/tweet_number_of_engagements/{self.feature_name}.csv.gz")
...
class TweetNumberOfEngagementPositive(TweetNumberOfEngagementsAbstract):
...
class TweetNumberOfEngagementNegative(TweetNumberOfEngagementsAbstract):
...
class TweetNumberOfEngagementLike(TweetNumberOfEngagementsAbstract):
...
class TweetNumberOfEngagementRetweet(TweetNumberOfEngagementsAbstract):
...
class TweetNumberOfEngagementReply(TweetNumberOfEngagementsAbstract):
...
class TweetNumberOfEngagementComment(TweetNumberOfEngagementsAbstract):
