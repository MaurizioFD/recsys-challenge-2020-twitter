...
def compute(train, test):
    dictionary = pd.DataFrame({"count": train.groupby("mapped_feature_creator_id").size()}).to_dict()["count"]
    result = pd.DataFrame(test["mapped_feature_creator_id"].map(lambda x: dictionary.get(x, 0)), index=test.index)
    return result

class CreatorNumberOfEngagementReceivedAbstract(GeneratedFeaturePickle, ABC):

    def __init__(self, dataset_id: str):

        super().__init__("creator_number_of_" + self._get_suffix() + "engagements_received", dataset_id)
        self.pck_path = pl.Path(
            f"{Feature.ROOT_PATH}/{self.dataset_id}/generated/creator_number_of_engagements_received/{self.feature_name}.pck.gz")
        self.csv_path = pl.Path(
            f"{Feature.ROOT_PATH}/{self.dataset_id}/generated/creator_number_of_engagements_received/{self.feature_name}.csv.gz")
...
class CreatorNumberOfEngagementReceivedPositive(CreatorNumberOfEngagementReceivedAbstract):
...
class CreatorNumberOfEngagementReceivedNegative(CreatorNumberOfEngagementReceivedAbstract):
...
class CreatorNumberOfEngagementReceivedLike(CreatorNumberOfEngagementReceivedAbstract):
...
class CreatorNumberOfEngagementReceivedRetweet(CreatorNumberOfEngagementReceivedAbstract):
...
class CreatorNumberOfEngagementReceivedReply(CreatorNumberOfEngagementReceivedAbstract):
...
class CreatorNumberOfEngagementReceivedComment(CreatorNumberOfEngagementReceivedAbstract):
